...
def info(string):
    print("\033[38m[{}]\033[0m".format(time.strftime("%H:%M:%S")) + set_color("[INFO] {}".format(string), level=20))


def debug(string):
    print("\033[38m[{}]\033[0m".format(time.strftime("%H:%M:%S")) + set_color("[DEBUG] {}".format(string), level=10))


def warn(string, minor=False):
    if not minor:
        print("\033[38m[{}]\033[0m".format(time.strftime("%H:%M:%S")) + set_color("[WARN] {}".format(string), level=30))
    else:
...
def fatal(string):
    print("\033[38m[{}]\033[0m".format(time.strftime("%H:%M:%S")) + set_color("[FATAL] {}".format(string), level=60))


def prompt(string):
    try:
        question = raw_input(
            "\033[38m[{}]\033[0m".format(time.strftime("%H:%M:%S")) + set_color("[PROMPT] {}: ".format(string), level=50)
        )
    except:
        quetion = input(
            "\033[38m[{}]\033[0m".format(time.strftime("%H:%M:%S")) + set_color("[PROMPT] {}: ".format(string), level=50)
